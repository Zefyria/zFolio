...
def main():
    ...
    for row in range(0,height):
        for column in range(0,width):
            try:
                if tachyon[row][column] == 'S': # top row only
                    tachyon[row+1][column] = '|'
                elif tachyon[row-1][column] == '|' and tachyon[row][column] == '^': # beam above, splitter
                    tachyon[row][column-1] = '|'
                    tachyon[row][column+1] = '|'
                    counter += 1
                elif tachyon[row-1][column] == '|' and tachyon[row][column] == '.': # beam above, no splitter
                    tachyon[row][column] = '|'
            except IndexError:
                print('probs bottom row')
